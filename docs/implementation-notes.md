...
3. Apply the `/api/storage-readiness` PostgreSQL/PostGIS schema, then wire documents, claims, events, event updates, and media assets to it.
...
- `/api/storage-readiness` exposes the durable event-store schema and configuration checks, but no database driver or runtime write path is active yet.

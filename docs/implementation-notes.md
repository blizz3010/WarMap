...
- Scheduled ingestion heartbeat: `vercel.json` schedules `/api/cron/ingest` daily. The cron route requires `CRON_SECRET`, runs the collector/extraction/review intake path for configured theaters, and returns counts plus source-link samples. Optional `INGESTION_STORE_PROVIDER=github` or `local-file` snapshot storage preserves sanitized review candidates between collector windows while PostgreSQL/PostGIS remains the target event store; `/api/intake-store-health` verifies the configured snapshot store read-only.

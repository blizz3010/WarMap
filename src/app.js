...
function claimLabelPolicyForItem(item, sources = []) {
  ...
  const eventType = String(item.extraction?.eventType || item.eventType || "").trim().toLowerCase();
  const category = String(item.extraction?.category || item.category || "").trim().toLowerCase();
